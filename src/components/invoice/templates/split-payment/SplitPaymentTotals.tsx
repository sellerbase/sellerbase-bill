...
type SplitPaymentTotalsProps = {
  items: InvoiceItem[];
};

export default function SplitPaymentTotals({ items }: SplitPaymentTotalsProps) {
  ...
  const totals = useMemo(() => {
    ...
    items.forEach(item => {
      ...
      const splitAmount = subtotal * splitRatio;

      result.total += subtotal;
      result.splitTotal += splitAmount;

      if (item.type === 'child_product') {
        result.product += splitAmount;
      } else if (item.type === 'option') {
        if (item.title.includes('検品')) {
          result.inspection += splitAmount;
        } else if (item.title.includes('作業')) {
          result.work += splitAmount;
        } else if (
          ...
          result.packaging += splitAmount;
        } else if (item.title.includes('配送') || item.title.includes('運送')) {
          result.shipping += splitAmount;
        }
      }
    });
  ...
  return (
    <div className="mb-6 bg-gray-50 rounded-lg p-4">
      <div className="mb-2">
        <h3 className="text-base font-semibold text-gray-900">合計金額（分割）</h3>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-2">
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">CNY（メイン）</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.splitTotal)}</div>
        </div>
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">USD</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.splitTotal * 0.14)}</div>
        </div>
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">JPY</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.splitTotal * 20.27)}</div>
        </div>
      </div>
      ...
      {showBreakdown && (
        <div className="mt-2">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white p-2 rounded-md shadow-sm">
              <div className="flex justify-between items-center text-xs">
                <span className="text-gray-600">分割前</span>
                <span className="font-medium text-gray-900">
