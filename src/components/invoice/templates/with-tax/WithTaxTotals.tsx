...
type WithTaxTotalsProps = {
  items: InvoiceItem[];
};

export default function WithTaxTotals({ items }: WithTaxTotalsProps) {
  ...
  const totals = useMemo(() => {
    ...
    items.forEach(item => {
      ...
      if (item.type === 'child_product') {
        result.product += totalWithTax;
      } else if (item.type === 'option') {
        if (item.title.includes('検品')) {
          result.inspection += totalWithTax;
        } else if (item.title.includes('作業')) {
          result.work += totalWithTax;
        } else if (
          ...
          result.packaging += totalWithTax;
        } else if (item.title.includes('配送') || item.title.includes('運送')) {
          result.shipping += totalWithTax;
        }
      }
    });
  ...
  return (
    <div className="mb-6 bg-gray-50 rounded-lg p-4">
      <div className="mb-2">
        <h3 className="text-base font-semibold text-gray-900">合計金額（税込）</h3>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-2">
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">CNY（メイン）</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.totalWithTax)}</div>
        </div>
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">USD</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.totalWithTax * 0.14)}</div>
        </div>
        <div className="bg-white p-2 rounded-md shadow-sm">
          <div className="text-xs text-gray-600 mb-0.5">JPY</div>
          <div className="text-lg font-bold text-gray-900">{formatCurrency(totals.totalWithTax * 20.27)}</div>
        </div>
      </div>
